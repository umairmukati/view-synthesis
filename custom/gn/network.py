...
class Net(nn.Module):
    ...
    def __init__(self, input_shape, minibatch_size, lfsize, batchAffine = False, bias = False):
        ...
        self.lfsize = lfsize
        self.dmax = 4
        nc_fh = nc_fd = 16; ng_fd = ng_fh = 2;
        nc_dh = nc_dd = 64; ng_dh = ng_dd = 4;
        nc_s = 128; ng_s = 8;
        ...
        self.fh_gn0 = nn.GroupNorm(ng_fh, nc_fh, affine=batchAffine)
        self.fh_gn1 = nn.GroupNorm(ng_fh, nc_fh, affine=batchAffine)
        self.fh_gn2 = nn.GroupNorm(ng_fh, nc_fh, affine=batchAffine)
        self.fh_gn3 = nn.GroupNorm(ng_fh, nc_fh, affine=batchAffine)
        self.fh_gn4 = nn.GroupNorm(ng_fh, nc_fh, affine=batchAffine)
        self.fh_gn5 = nn.GroupNorm(ng_fh, nc_fh, affine=batchAffine)
        ...
        self.fd_gn0 = nn.GroupNorm(ng_fd, nc_fd, affine=batchAffine)
        self.fd_gn1 = nn.GroupNorm(ng_fd, nc_fd, affine=batchAffine)
        self.fd_gn2 = nn.GroupNorm(ng_fd, nc_fd, affine=batchAffine)
        self.fd_gn3 = nn.GroupNorm(ng_fd, nc_fd, affine=batchAffine)
        self.fd_gn4 = nn.GroupNorm(ng_fd, nc_fd, affine=batchAffine)
        self.fd_gn5 = nn.GroupNorm(ng_fd, nc_fd, affine=batchAffine)
        ...
        self.dh_gn0 = nn.GroupNorm(ng_dh, nc_dh, affine=batchAffine)
        self.dh_gn1 = nn.GroupNorm(ng_dh, nc_dh, affine=batchAffine)
        self.dh_gn2 = nn.GroupNorm(ng_dh, nc_dh, affine=batchAffine)
        self.dh_gn3 = nn.GroupNorm(ng_dh, nc_dh, affine=batchAffine)
        self.dh_gn4 = nn.GroupNorm(ng_dh, nc_dh//2, affine=batchAffine)
        self.dh_gn5 = nn.GroupNorm(ng_dh, nc_dh//2, affine=batchAffine)
        ...
        self.dd_gn0 = nn.GroupNorm(ng_dd, nc_dd, affine=batchAffine)
        self.dd_gn1 = nn.GroupNorm(ng_dd, nc_dd, affine=batchAffine)
        self.dd_gn2 = nn.GroupNorm(ng_dd, nc_dd, affine=batchAffine)
        self.dd_gn3 = nn.GroupNorm(ng_dd, nc_dd, affine=batchAffine)
        self.dd_gn4 = nn.GroupNorm(ng_dd, nc_dd//2, affine=batchAffine)
        self.dd_gn5 = nn.GroupNorm(ng_dd, nc_dd//2, affine=batchAffine)
        ...
        self.s_gn0 = nn.GroupNorm(ng_s, nc_s//2, affine=batchAffine)
        self.s_gn1 = nn.GroupNorm(ng_s, nc_s, affine=batchAffine)
        self.s_gn2 = nn.GroupNorm(ng_s, nc_s, affine=batchAffine)
        self.s_gn3 = nn.GroupNorm(ng_s, nc_s, affine=batchAffine)
        self.s_gn4 = nn.GroupNorm(ng_s, nc_s//2, affine=batchAffine)
        self.s_gn5 = nn.GroupNorm(ng_s, nc_s//4, affine=batchAffine)
                
    # Features Horizontal CNN
    def fh_cnn(self, x):
       
        x = self.fh_gn0(F.elu(self.fh_conv0(x)))
        x = self.fh_gn1(F.elu(self.fh_conv1(x)))
        x = self.fh_gn2(F.elu(self.fh_conv2(x)))
        x_conv2 = x
        x = self.fh_gn3(F.elu(self.fh_conv3(x)))
        x = self.fh_gn4(F.elu(self.fh_conv4(x)))
        x_conv4 = x + x_conv2
        ...
        x = self.fh_gn5(F.elu(self.fh_conv5(x)))
    ...
    def fd_cnn(self, x):
        ...
        x = self.fd_gn0(F.elu(self.fd_conv0(x)))
        x = self.fd_gn1(F.elu(self.fd_conv1(x)))
        x = self.fd_gn2(F.elu(self.fd_conv2(x)))
        x_conv2 = x
        x = self.fd_gn3(F.elu(self.fd_conv3(x)))
        x = self.fd_gn4(F.elu(self.fd_conv4(x)))
        x_conv4 = x + x_conv2
        ...
        x = self.fd_gn5(F.elu(self.fd_conv5(x)))
    ...
    def dh_cnn(self, x):
        ...
        x = self.dh_gn0(F.elu(self.dh_conv0(x)))
        x = self.dh_gn1(F.elu(self.dh_conv1(x)))
        x = self.dh_gn2(F.elu(self.dh_conv2(x)))
        x = self.dh_gn3(F.elu(self.dh_conv3(x)))
        x = self.dh_gn4(F.elu(self.dh_conv4(x)))
        x = self.dh_gn5(F.elu(self.dh_conv5(x)))
    ...
    def dd_cnn(self, x):
        ...
        x = self.dd_gn0(F.elu(self.dd_conv0(x)))
        x = self.dd_gn1(F.elu(self.dd_conv1(x)))
        x = self.dd_gn2(F.elu(self.dd_conv2(x)))
        x = self.dd_gn3(F.elu(self.dd_conv3(x)))
        x = self.dd_gn4(F.elu(self.dd_conv4(x)))
        x = self.dd_gn5(F.elu(self.dd_conv5(x)))
    ...
    def s_cnn(self, x):
        ...
        x = self.s_gn0(F.elu(self.s_conv0(x)))
        x = self.s_gn1(F.elu(self.s_conv1(x)))
        x = self.s_gn2(F.elu(self.s_conv2(x)))
        x = self.s_gn3(F.elu(self.s_conv3(x)))
        x = self.s_gn4(F.elu(self.s_conv4(x)))
        x = self.s_gn5(F.elu(self.s_conv5(x)))
...
#         FH = self.fh_cnn(torch.cat((torch.cat((x_00, x_01, x_10, x_11), 0),torch.cat((x_00, x_01, x_10, x_11), 0).permute(0,1,3,2)),0))
#         FH_00  = FH[:3];    FH_01  = FH[3:6];   FH_10  = FH[6:9];   FH_11  = FH[9:12]; 
#         FHT_00 = FH[12:15]; FHT_01 = FH[15:18]; FHT_10 = FH[18:21]; FHT_11 = FH[21:24]; 
                
#         FD = self.fd_cnn(torch.cat((x_00, x_01.flip(2), x_10.flip(2), x_11),0))
#         FD_00  = FD[:3];    FDT_01  = FD[3:6];   FDT_10  = FD[6:9];   FD_11  = FD[9:12]; 
        
#         # Estimating disparities
#         dH = self.dh_cnn(torch.cat((torch.cat((FH_00, FH_01), 1), torch.cat((FH_10, FH_11), 1), \
#                                     torch.cat((FHT_00, FHT_10), 1), torch.cat((FHT_01, FHT_11), 1)),0))
#         d_TH = dH[:3];                   d_BH = dH[3:6]; 
#         d_LV = dH[6:9].permute(0,1,3,2); d_RV = dH[9:12].permute(0,1,3,2); 
        
#         dD = self.dd_cnn(torch.cat((torch.cat((FD_00, FD_11), 1), torch.cat((FDT_10, FDT_01), 1)),0))
#         d_BD = dD[:3]; d_FD = dD[3:6].flip(2);
