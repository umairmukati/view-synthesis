...
class Net(nn.Module):
    ...
    def __init__(self, input_shape, minibatch_size, lfsize, batchAffine = False, bias = True):
        ...
        self.f_bn0 = nn.GroupNorm(2, 32, affine=batchAffine)
        self.f_bn1 = nn.GroupNorm(2, 32, affine=batchAffine)
        self.f_bn2 = nn.GroupNorm(2, 32, affine=batchAffine)
        self.f_bn3 = nn.GroupNorm(2, 32, affine=batchAffine)
        self.f_bn4 = nn.GroupNorm(2, 32, affine=batchAffine)
        self.f_bn5 = nn.GroupNorm(2, 32, affine=batchAffine)
        ...
        self.d_bn0 = nn.GroupNorm(4, 128, affine=batchAffine)
        self.d_bn1 = nn.GroupNorm(4, 128, affine=batchAffine)
        self.d_bn2 = nn.GroupNorm(4, 128, affine=batchAffine)
        self.d_bn3 = nn.GroupNorm(4, 128, affine=batchAffine)
        self.d_bn4 = nn.GroupNorm(4, 64,  affine=batchAffine)
        self.d_bn5 = nn.GroupNorm(4, 64,  affine=batchAffine)
        ...
        self.s_bn0 = nn.GroupNorm(8, 64,  affine=batchAffine)
        self.s_bn1 = nn.GroupNorm(8, 128, affine=batchAffine)
        self.s_bn2 = nn.GroupNorm(8, 128, affine=batchAffine)
        self.s_bn3 = nn.GroupNorm(8, 128, affine=batchAffine)
        self.s_bn4 = nn.GroupNorm(8, 64,  affine=batchAffine)
        self.s_bn5 = nn.GroupNorm(8, 32,  affine=batchAffine)
                
    # Features CNN
    def fcnn(self, x):
       
        x = F.elu(self.f_bn0(self.f_conv0(x)))
        x = F.elu(self.f_bn1(self.f_conv1(x)))
        x = F.elu(self.f_bn2(self.f_conv2(x)))
        x_conv2 = x
        x = F.elu(self.f_bn3(self.f_conv3(x)))
        x = F.elu(self.f_bn4(self.f_conv4(x)))
        x_conv4 = x + x_conv2
        ...
        x = F.elu(self.f_bn5(self.f_conv5(x)))
    ...
    def dcnn(self, x):
        ...
        x = F.elu(self.d_bn0(self.d_conv0(x)))
        x = F.elu(self.d_bn1(self.d_conv1(x)))
        x = F.elu(self.d_bn2(self.d_conv2(x)))
        x = F.elu(self.d_bn3(self.d_conv3(x)))
        x = F.elu(self.d_bn4(self.d_conv4(x)))
        x = F.elu(self.d_bn5(self.d_conv5(x)))
    ...
    def scnn(self, x):
        ...
        x = F.elu(self.s_bn0(self.s_conv0(x)))
        x = F.elu(self.s_bn1(self.s_conv1(x)))
        x = F.elu(self.s_bn2(self.s_conv2(x)))
        x = F.elu(self.s_bn3(self.s_conv3(x)))
        x = F.elu(self.s_bn4(self.s_conv4(x)))
        x = F.elu(self.s_bn5(self.s_conv5(x)))
    ...
    def forward(self, x, p, q):
        ...
        feat = self.fcnn(torch.cat((torch.cat((x_00, self.P, self.Q), 1),\
                                 torch.cat((x_01, self.P, self.Q), 1),\
                                 torch.cat((x_10, self.P, self.Q), 1),\
                                 torch.cat((x_11, self.P, self.Q), 1)),0))
        
        F_00  = feat[self.batch_size*0:self.batch_size*1];    
        F_01  = feat[self.batch_size*1:self.batch_size*2];   
        F_10  = feat[self.batch_size*2:self.batch_size*3];
        F_11  = feat[self.batch_size*3:self.batch_size*4]; 
        
        
        # input_channels = 32 + 32 + 32 + 32 + 1 + 1
        disp = self.dcnn(torch.cat((F_00, F_01, F_10, F_11, self.P, self.Q), 1))
                
        D_00 = disp[:,0,:,:].unsqueeze(3).repeat(1,1,1,2)*self.D[:,0]
        D_01 = disp[:,1,:,:].unsqueeze(3).repeat(1,1,1,2)*self.D[:,1]
        D_10 = disp[:,2,:,:].unsqueeze(3).repeat(1,1,1,2)*self.D[:,2]
        D_11 = disp[:,3,:,:].unsqueeze(3).repeat(1,1,1,2)*self.D[:,3]
        ...
        M = self.scnn(torch.cat((w_00, w_01, w_10, w_11, disp, self.P, self.Q), 1))
